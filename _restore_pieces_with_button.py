# -*- coding: utf-8 -*-
"""Restore documents tab: main OPS card + New/Close + conditional upload (undo revert to 3 cards)."""
path = r"C:\Users\Personnel\Desktop\project\core_bankink_dashboard\src\app\customers\_detail\CustomerDetailPage.tsx"
...
i2 = s.find('{activeTab === "compliance"', i0)
if i2 < 0:
    raise SystemExit("compliance start not found")
# end of pieces block: closing `)}` for the outer `activeTab === "pieces" && (` — last `)}` before compliance
chunk = s[i0:i2]
last_close = chunk.rfind("\t\t\t\t)}")
if last_close < 0:
    raise SystemExit("closing )} not found in pieces chunk")
i1 = i0 + last_close + len("\t\t\t\t)}")

form_part = r"""									{(docType === "ID_CARD" || docType === "PASSPORT") && (
...
list_open = r"""							<div className="overflow-hidden rounded-ops-lg border border-ops-border bg-ops-surface-muted/30">
...
list_rest = s[s.find("{documents.length === 0 ? (", i0) : i2]
if "{documents.length === 0 ? (" not in s[i0:i2]:
    raise SystemExit("documents list marker missing")

idx_docs = s.find("{documents.length === 0 ? (", i0)
list_rest = s[idx_docs:]  # from {documents... to end of file - TOO MUCH

# list_rest should only be from {documents to i1 (end of pieces before compliance)
...
# Fix closings: list section needs +1 outer div for main OPS shell (4 closing divs after ternary)
needle = "\t\t\t\t\t\t\t\t)}\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t)}"
repl = "\t\t\t\t\t\t\t\t)}\n\t\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t\t</div>\n\t\t\t\t\t\t</div>\n\t\t\t\t\t</div>\n\t\t\t\t)}"
if needle not in s2:
    raise SystemExit("closing needle not found for expansion")
s2 = s2.replace(needle, repl, 1)

with open(path, "w", encoding="utf-8") as f:
    f.write(s2)
print("pieces block replaced", i0, i1, "len", i1 - i0)
