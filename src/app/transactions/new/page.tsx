...
export default function NewTransactionPage() {
		...
		async function loadClientAccounts() {
			if (selectedClientId && selectedClientId !== "") {
				try {
					...
					setClientAccounts(data.filter((acc) => acc.status === "ACTIVE"));
					
					// Si un accountId est dans l'URL et correspond à un compte du client, le pré-sélectionner
					if (accountIdParam) {
				...
				setClientAccounts([]);
				// Si pas de client sélectionné, charger tous les comptes actifs (fallback)
	...
	const selectedAccount = (clientAccounts.length > 0 ? clientAccounts : accounts).find(a => a.id === formData.accountId);
			...
			<div>
				...
				<div className="flex items-center gap-4">
					...
					<div>
						...
						<p className="text-gray-600 mt-1">Créer une nouvelle transaction bancaire</p>
					</div>
				</div>
			</div>
				...
				<form onSubmit={handleSubmit} className="p-6">
					<div className="space-y-6">
							...
							{selectedClient && selectedClient.status !== "VERIFIED" && (
								<div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
									...
									<p className="text-sm text-red-800">
										Le client doit être vérifié (VERIFIED) pour créer une transaction
									</p>
								</div>
							)}
						</div>
						...
						<div>
							...
							{selectedClientId && clientAccounts.length === 0 ? (
								...
								<select
									...
									required
									disabled={selectedClientId && clientAccounts.length === 0}
								>
									<option value={0}>
										{selectedClientId 
											? "Sélectionner un compte du client" 
											: "Sélectionner un compte (sélectionnez d'abord un client)"}
									</option>
									{(clientAccounts.length > 0 ? clientAccounts : accounts).map((acc) => (
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div>
								...
								<Input
									...
									placeholder="XAF (par défaut: devise du compte)"
								/>
							</div>
						...
						<div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
							...
							<Button 
								type="submit" 
								disabled={loading || !formData.accountId || (selectedClientId && clientAccounts.length === 0)}
