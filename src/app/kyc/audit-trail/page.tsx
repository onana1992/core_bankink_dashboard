...
export default function KycAuditTrailPage() {
	...
	useEffect(() => {
		...
		(async () => {
			...
			setError(null);
			try {
				const data = await auditApi.getKycAuditEvents({
					userId: userIdParam,
	...
	return (
		<div className={OPS_PAGE_STACK}>
			<OpsPageHeader
				title={t("kycAuditTrailPage.title")}
				description={t("kycAuditTrailPage.subtitle")}
				actions={
					<Link
						href="/audit"
					...
					>
						{t("kycAuditTrailPage.linkGlobalAudit")}
					</Link>
				}
			/>
			...
			<div className="flex flex-col gap-3 rounded-ops-xl border border-ops-border bg-ops-surface p-4 sm:flex-row sm:flex-wrap sm:items-end">
				<div className="min-w-[8rem] flex-1">
					<label className="mb-1 block text-xs font-medium text-ops-fg-muted">{t("kycAuditTrailPage.userIdPlaceholder")}</label>
					<Input
						type="number"
						min={1}
				...
				<div className="min-w-[10rem] flex-1">
					<label className="mb-1 block text-xs font-medium text-ops-fg-muted">{t("kycAuditTrailPage.fromDate")}</label>
					<Input
						type="datetime-local"
				...
				<div className="min-w-[10rem] flex-1">
					<label className="mb-1 block text-xs font-medium text-ops-fg-muted">{t("kycAuditTrailPage.toDate")}</label>
					<Input
						type="datetime-local"
			...
			<AuditEventsTable
				...
				pageSize={pageSize}
				resultsHeading={t("kycAuditTrailPage.resultsHeading")}
				onPageChange={p => {
					setCurrentPage(p);
