...
export default function LedgerAccountsPage() {
	...
	async function loadAccounts() {
		...
		try {
			...
			setError(e?.message ?? "Erreur lors du chargement des comptes GL");
		} finally {
			setLoading(false);
		}
	}
	...
	function validateForm(): boolean {
		...
		if (!form.chartOfAccountCode) {
			errors.chartOfAccountCode = "Le compte comptable est requis";
		} else {
			const chartAccount = chartOfAccounts.find(c => c.code === form.chartOfAccountCode);
			if (!chartAccount) {
				errors.chartOfAccountCode = "Le compte comptable n'existe pas";
			} else if (!chartAccount.isActive) {
				errors.chartOfAccountCode = "Le compte comptable doit être actif";
			} else if (chartAccount.accountType !== form.accountType) {
				errors.accountType = "Le type doit correspondre au type du compte comptable";
			}
		}
	...
	return (
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-3xl font-bold text-gray-900">Comptes du Grand Livre</h1>
					<p className="text-gray-600 mt-1">Gestion et configuration des comptes GL</p>
				</div>
				<div className="flex gap-3">
					...
					<Button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2">
						...
						{showForm ? "Annuler" : "Nouveau compte GL"}
					</Button>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
				...
				<div className="bg-gradient-to-br from-purple-50 to-purple-100 p-5 rounded-xl shadow-sm border border-purple-200">
					<div className="flex items-center justify-between">
						<div>
							<div className="text-sm font-medium text-purple-700 mb-1">Devises</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
					<div>
						...
						<div className="relative">
							<Input
								placeholder="Code, nom ou compte comptable..."
								value={searchQuery}
				...
				<div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
					<div className="flex justify-between items-center mb-4">
						<h3 className="text-lg font-semibold text-gray-900">
							{editingAccount ? "Modifier le compte GL" : "Nouveau compte GL"}
						</h3>
						<Button
							type="button"
							variant="outline"
							...
							<div>
								...
								{form.chartOfAccountCode && (
									<p className="text-xs text-gray-500 mt-1">
										Type automatique: {getAccountTypeLabel(form.accountType)}
									</p>
								)}
							</div>
							<div>
								...
								{form.chartOfAccountCode && (
									<p className="text-xs text-gray-500 mt-1">Défini par le compte comptable</p>
								)}
							</div>
							<div>
			...
			{loading ? (
				<div className="bg-white p-12 rounded-xl shadow-sm border border-gray-200 text-center">
					...
					<p className="mt-4 text-gray-600">Chargement des comptes GL...</p>
				</div>
				...
				<div className="bg-white p-12 rounded-xl shadow-sm border border-gray-200 text-center">
					...
					<p className="text-gray-500 text-lg font-medium">Aucun compte GL trouvé</p>
						...
						<table className="min-w-full divide-y divide-gray-200">
							<thead className="bg-gray-50">
								<tr>
									...
									<th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Compte comptable</th>
